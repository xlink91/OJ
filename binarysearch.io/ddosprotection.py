...
class Solution:
    def solve(self, requests, u, g):
        ...
        for rq in requests:
            ...
            if _id not in local_qt_map:
                local_qt_map[_id] = deque()
            gl = self.check_global(rq, global_qt)
            lc = self.check_local(rq, local_qt_map)
            if gl and lc:
                req_p += 1
                self.insert_qt(rq, global_qt, g)
                self.insert_qt(rq, local_qt_map[_id], u)
        return req_p

    def insert_qt(self, e, qt, mx):
        if len(qt) == mx:
            qt.popleft()
        qt.append(e)
    ...
    def check_global(self, rq, g, gl) -> bool:
        ...
        _, sec = rq
        _, tp_sec = gl[0]
        if tp_sec - sec < 60 and len(gl) >= g:
            return False
        return True

    def check_local(self, rq, l, lc):
        if not lc:
            return True

        _, sec = l
        _, tp_sec = lc[0]
        if tp_sec - sec < 60 and len(lc) >= l:
            return False
        return True
